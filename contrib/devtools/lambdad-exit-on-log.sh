...
DEFAULT_LAMBDAD="${TOPLEVEL}/build/src/lambdad"
...
help_message() {
  ...
  echo "LAMBDAD              Default: ${DEFAULT_LAMBDAD}"
  echo "LOG_FILE              Default: ${DEFAULT_LOG_FILE}"
  set -x
}

: "${LAMBDAD:=${DEFAULT_LAMBDAD}}"
...
read -ar LAMBDAD_PARAMS <<< "${LAMBDAD_PARAMS}"
START_TIME=$(date +%s)
if [ "${#LAMBDAD_PARAMS[@]}" -gt 0 ]; then
  "${LAMBDAD}" "${LAMBDAD_PARAMS[@]}" &
else
  "${LAMBDAD}" &
fi
LAMBDAD_PID=$!
...
(
  ...
  log_subshell_cleanup() {
    echo "Cleaning up lambda daemon (PID: ${LAMBDAD_PID})."
    kill ${LAMBDAD_PID}
  }
...
if [ "${LAMBDAD_EXIT_CODE}" -ne "0" ]; then
  echo "lambdad exited unexpectedly with code: ${LAMBDAD_EXIT_CODE}"
  exit ${LAMBDAD_EXIT_CODE}
fi
