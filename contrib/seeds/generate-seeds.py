...
The output will be four data structures with the peers in binary format:
...
def process_nodes(g, f, structname, defaultport):
    g.write('static const SeedSpec6 {}[] = {{\n'.format(structname))
    first = True
    for line in f:
        comment = line.find('#')
        if comment != -1:
        ...
        (_, port) = parse_spec(line, None)
        hoststr = line
        if port is None:
            hoststr += ":{}".format(defaultport)
        g.write('    {{ "{}" }}'.format(hoststr))
    g.write('\n};\n')
...
def main():
    ...
    g = sys.stdout
    indir = sys.argv[1]
    g.write('#ifndef LAMBDA_CHAINPARAMSSEEDS_H\n')
    g.write('#define LAMBDA_CHAINPARAMSSEEDS_H\n')
    g.write('/**\n')
    g.write(' * List of fixed seed nodes for the Lambda main and test networks\n')
    g.write(' * @generated by contrib/seeds/generate-seeds.py\n')
    g.write(' *\n')
    g.write(' * Each string must be numeric IPv4 dot-notation or numeric\n')
    ...
    with open(os.path.join(indir, 'nodes_testnet3.txt'), 'r', encoding="utf8") as f:
        process_nodes(g, f, 'pnSeed6_testnet3', 19333)
    g.write('\n')
    with open(os.path.join(indir, 'nodes_testnet4.txt'), 'r', encoding="utf8") as f:
        process_nodes(g, f, 'pnSeed6_testnet4', 29333)
    g.write('\n')
    with open(os.path.join(indir, 'nodes_scalenet.txt'), 'r', encoding="utf8") as f:
        process_nodes(g, f, 'pnSeed6_scalenet', 39333)
    g.write('#endif // LAMBDA_CHAINPARAMSSEEDS_H\n')
