...
"""Test lambdad with different proxy configuration.

Test plan:
- Start lambdad's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on lambdad side:
...
- Create lambdads that connect to them
- Manipulate the lambdads using addnode (onetry) an observe effects
...
class ProxyTest(LambdaTestFramework):
    ...
    def setup_nodes(self):
        ...
        args = [
            ...
            ['-listen', '-proxy={}:{}'.format(
             ...
             '-onion={}:{}'.format(
                 self.conf2.addr[0], self.conf2.addr[1]), '-proxyrandomize=0'],
            ['-listen', '-proxy={}:{}'.format(
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: lambdad's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert isinstance(cmd, Socks5Command)
            # Note: lambdad's SOCKS5 implementation only sends atyp
        ...
        if test_onion:
            ...
            node.addnode("lambdaostk4e4re.onion:9333", "onetry")
            cmd = proxies[2].queue.get()
            assert isinstance(cmd, Socks5Command)
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, b"lambdaostk4e4re.onion")
