...
//! so lambda should never create different databases with the same fileid, but
...
void CheckUniqueFileid(const BerkeleyEnvironment &env,
    ...
    for (const auto &item : env.m_fileids) {
        if (fileid == item.second && &fileid != &item.second) {
            throw std::runtime_error(strprintf("BerkeleyBatch: Can't open database %s (duplicates fileid %s from %s)",
...
bool BerkeleyEnvironment::Open(bool retry) {
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another "
                  "instance of lambda may be using it.\n",
                  strPath);
        return false;
    }
...
BerkeleyBatch::BerkeleyBatch(BerkeleyDatabase &database, const char *pszMode,
...

    {
        ...
        if (pdb == nullptr) {
            ...
            // lambda from opening the same data file through another
