...
static bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/lambda.conf are parsed in qt/lambda.cpp's
    // main()
    SetupServerArgs();
    std::string error;
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr,
                        ...
                        "see lambdad -h for a list of options.\n",
                        argv[i]);
                return false;
            }
        }

        // -server defaults to true for lambdad but not for the GUI so do this
        // here
...
int main(int argc, char *argv[]) {
    ...
    // Connect lambdad signal handlers
    noui_connect();
