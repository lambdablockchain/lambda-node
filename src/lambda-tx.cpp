...
static int AppInitRawTx(int argc, char *argv[], Config &config) {
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage =
            PACKAGE_NAME " lambda-tx utility version " + FormatFullVersion() +
            "\n\n" +
            "Usage:  lambda-tx [options] <hex-tx> [commands]  Update "
            "hex-encoded lambda transaction\n" +
            "or:     lambda-tx [options] -create [commands]   Create "
            "hex-encoded lambda transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char *argv[], const Config &config, const CChainParams &chainParams) {
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded lambda transaction
