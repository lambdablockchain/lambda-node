...
public:
    explicit CAddrManTest(bool makeDeterministic = true, std::vector<bool> asmap = {}) {
...
static std::vector<bool> FromBytes(const uint8_t *source, size_t vector_size) {
    std::vector<bool> result(vector_size);
    for (size_t byte_i = 0; byte_i < vector_size / 8; ++byte_i) {
        auto cur_byte = source[byte_i];
...
BOOST_AUTO_TEST_CASE(addrman_simple) {
    ...
    CService addr1 = ResolveService("250.1.1.1", 9333);
    BOOST_CHECK(addrman.Add(CAddress(addr1, NODE_NONE), source));
    BOOST_CHECK_EQUAL(addrman.size(), 1U);
    CAddrInfo addr_ret1 = addrman.Select();
    BOOST_CHECK_EQUAL(addr_ret1.ToString(), "250.1.1.1:9333");

    // Test: Does IP address deduplication work correctly.
    //  Expected dup IP should not be added.
    CService addr1_dup = ResolveService("250.1.1.1", 9333);
    ...
    // success.
    CService addr2 = ResolveService("250.1.1.2", 9333);
    ...
    vAddr.push_back(CAddress(ResolveService("250.1.1.3", 9333), NODE_NONE));
    vAddr.push_back(CAddress(ResolveService("250.1.1.4", 9333), NODE_NONE));
...
BOOST_AUTO_TEST_CASE(addrman_ports) {
    ...
    CService addr1_port = ResolveService("250.1.1.1", 9334);
    BOOST_CHECK(!addrman.Add(CAddress(addr1_port, NODE_NONE), source));
    BOOST_CHECK_EQUAL(addrman.size(), 1U);
    CAddrInfo addr_ret2 = addrman.Select();
    BOOST_CHECK_EQUAL(addr_ret2.ToString(), "250.1.1.1:9333");
    ...
    BOOST_CHECK_EQUAL(addr_ret3.ToString(), "250.1.1.1:9333");
}
...
BOOST_AUTO_TEST_CASE(addrman_select) {
    ...
    CService addr2 = ResolveService("250.3.1.1", 9333);
    CService addr3 = ResolveService("250.3.2.2", 9999);
    CService addr4 = ResolveService("250.3.3.3", 9999);

    BOOST_CHECK(addrman.Add(CAddress(addr2, NODE_NONE), ResolveService("250.3.1.1", 9333)));
    BOOST_CHECK(addrman.Add(CAddress(addr3, NODE_NONE), ResolveService("250.3.1.1", 9333)));
    BOOST_CHECK(addrman.Add(CAddress(addr4, NODE_NONE), ResolveService("250.4.1.1", 9333)));

    // Add three addresses to tried table.
    CService addr5 = ResolveService("250.4.4.4", 9333);
    CService addr6 = ResolveService("250.4.5.5", 7777);
    CService addr7 = ResolveService("250.4.6.6", 9333);

    BOOST_CHECK(addrman.Add(CAddress(addr5, NODE_NONE), ResolveService("250.3.1.1", 9333)));
    addrman.Good(CAddress(addr5, NODE_NONE));
    BOOST_CHECK(addrman.Add(CAddress(addr6, NODE_NONE), ResolveService("250.3.1.1", 9333)));
    addrman.Good(CAddress(addr6, NODE_NONE));
    BOOST_CHECK(addrman.Add(CAddress(addr7, NODE_NONE), ResolveService("250.1.1.3", 9333)));
...
BOOST_AUTO_TEST_CASE(addrman_find) {
    ...
    CAddress addr1 = CAddress(ResolveService("250.1.2.1", 9333), NODE_NONE);
    CAddress addr2 = CAddress(ResolveService("250.1.2.1", 9999), NODE_NONE);
    CAddress addr3 = CAddress(ResolveService("251.255.2.1", 9333), NODE_NONE);
    ...
    BOOST_REQUIRE(info1);
    BOOST_CHECK_EQUAL(info1->ToString(), "250.1.2.1:9333");
    ...
    BOOST_REQUIRE(info3);
    BOOST_CHECK_EQUAL(info3->ToString(), "251.255.2.1:9333");
}
...
BOOST_AUTO_TEST_CASE(addrman_create) {
    ...
    BOOST_CHECK_EQUAL(pinfo->ToString(), "250.1.2.1:9333");

    CAddrInfo *info2 = addrman.Find(addr1);
    BOOST_CHECK_EQUAL(info2->ToString(), "250.1.2.1:9333");
}
...
BOOST_AUTO_TEST_CASE(addrman_getaddr) {
    ...
    CAddress addr1 = CAddress(ResolveService("250.250.2.1", 9333), NODE_NONE);
    addr1.nTime = GetAdjustedTime(); // Set time so isTerrible = false
    CAddress addr2 = CAddress(ResolveService("250.251.2.2", 9999), NODE_NONE);
    addr2.nTime = GetAdjustedTime();
    CAddress addr3 = CAddress(ResolveService("251.252.2.3", 9333), NODE_NONE);
    addr3.nTime = GetAdjustedTime();
    CAddress addr4 = CAddress(ResolveService("252.253.3.4", 9333), NODE_NONE);
    addr4.nTime = GetAdjustedTime();
    CAddress addr5 = CAddress(ResolveService("252.254.4.5", 9333), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_tried_bucket_legacy) {
    ...
    CAddress addr1 = CAddress(ResolveService("250.1.1.1", 9333), NODE_NONE);
    ...
    // Test: IP addresses in the same /16 prefix should never get more than 8 buckets with legacy grouping
    ...
    // Test: IP addresses in the different /16 prefix should map to more than 8 buckets with legacy grouping
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_new_bucket_legacy) {
    ...
    // Test: IP addresses in the same source groups should map to NO MORE than 64 buckets.
    ...
    // Test: IP addresses in the different source groups should map to MORE than 64 buckets.
    BOOST_CHECK(buckets.size() > 64);
}
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_tried_bucket) {
    ...
    std::set<int> buckets;
    for (int j = 0; j < 255; j++) {
        CAddrInfo infoj = CAddrInfo(CAddress(ResolveService("101." + std::to_string(j) + ".1.1"), NODE_NONE),
    ...
    buckets.clear();
    for (int j = 0; j < 255; j++) {
        CAddrInfo infoj = CAddrInfo(CAddress(ResolveService("250." + std::to_string(j) + ".1.1"), NODE_NONE),
                                    ResolveIP("250." + std::to_string(j) + ".1.1"));
        int bucket = infoj.GetTriedBucket(nKey1, asmap);
        buckets.insert(bucket);
    }
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_new_bucket) {
    ...
    std::set<int> buckets;
    for (int i = 0; i < 255; i++) {
        CAddrInfo infoi = CAddrInfo(CAddress(ResolveService("250.1.1." + std::to_string(i)), NODE_NONE),
                                    ResolveIP("250.1.1." + std::to_string(i)));
        int bucket = infoi.GetNewBucket(nKey1, asmap);
        buckets.insert(bucket);
    }
    ...
    for (int j = 0; j < 4 * 255; j++) {
        CAddrInfo infoj =
            CAddrInfo(CAddress(ResolveService(std::to_string(250 + (j / 255)) + "." + std::to_string(j % 256) + ".1.1"),
                               NODE_NONE),
                      ResolveIP("251.4.1.1"));
        int bucket = infoj.GetNewBucket(nKey1, asmap);
        buckets.insert(bucket);
    ...
    for (int p = 0; p < 255; p++) {
        CAddrInfo infoj =
            CAddrInfo(CAddress(ResolveService("250.1.1.1"), NODE_NONE), ResolveIP("101." + std::to_string(p) + ".1.1"));
        int bucket = infoj.GetNewBucket(nKey1, asmap);
        buckets.insert(bucket);
    }
    ...
    for (int p = 0; p < 255; p++) {
        CAddrInfo infoj =
            CAddrInfo(CAddress(ResolveService("250.1.1.1"), NODE_NONE), ResolveIP("250." + std::to_string(p) + ".1.1"));
        int bucket = infoj.GetNewBucket(nKey1, asmap);
        buckets.insert(bucket);
    }
