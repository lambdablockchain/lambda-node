...
BOOST_AUTO_TEST_CASE(multisig_verify) {
    ...
    BOOST_CHECK(VerifyScript(s, a_and_b, flags, MutableTransactionSignatureChecker(&txTo[0], 0, amount), null_context, &err));
    BOOST_CHECK_MESSAGE(err == ScriptError::OK, ScriptErrorString(err));

    for (int i = 0; i < 4; i++) {
        keys.assign(1, key[i]);
        s = sign_multisig(a_and_b, keys, txTo[0], 0);
        BOOST_CHECK_MESSAGE( ! VerifyScript(s, a_and_b, flags, MutableTransactionSignatureChecker(&txTo[0], 0, amount), null_context, &err),
            strprintf("a&b 1: %d", i));
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i == 0 || i == 1) {
            BOOST_CHECK_MESSAGE(VerifyScript(s, a_or_b, flags,
                                    MutableTransactionSignatureChecker(&txTo[1], 0, amount), null_context, &err),
                                strprintf("a|b: %d", i));
            BOOST_CHECK_MESSAGE(err == ScriptError::OK, ScriptErrorString(err));
        } else {
            BOOST_CHECK_MESSAGE( ! VerifyScript(s, a_or_b, flags,
    ...
    s.clear();
    s << OP_0 << OP_1;
    BOOST_CHECK( ! VerifyScript(s, a_or_b, flags, MutableTransactionSignatureChecker(&txTo[1], 0, amount), null_context, &err));
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (i < j && i < 3 && j < 3) {
                BOOST_CHECK_MESSAGE(VerifyScript(s, escrow, flags,
                        MutableTransactionSignatureChecker(&txTo[2], 0, amount), null_context, &err),
