...
static void initTranslations(QTranslator &qtTranslatorBase,
    ...
    // Load e.g. lambda_de.qm (shortcut "de" needs to be defined in
    // lambda.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. lambda_de_DE.qm (shortcut "de_DE" needs to be defined in
    // lambda.qrc)
...
int GuiMain(int argc, char *argv[]) {
    ...
    /// 0. Parse lambda-qt command-line options.
    ...
    /// configuration)
    Q_INIT_RESOURCE(lambda);
    Q_INIT_RESOURCE(lambda_locale);
